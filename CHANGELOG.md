...
修复：
- 横排：`1/4 em`、`a/b` 这类纯西文串里，分隔号 `/` 左右各被加了一个 1/4em 中西间距 —— `/` `·` `–` `——` `…` 都登记在标点表里、`kind` 为 `cjk_punct`，而横排后端把 `cjk_punct` 一律当作「中文一侧」自行判断边界，于是整行没有一个汉字也照加。中西间距是「汉字↔西文」规则，判断权归共享层的 `cjk_western.takes_spacing`（HR5：clreq 规则只写在 shared/），竖排后端一直用的就是它——横排后端此前重写了一份且结论相反，同一段文字横竖排间距因此不一致
