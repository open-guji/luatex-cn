...
--
-- 【模块功能 / Module Purpose】
-- 本模块是 cn_banxin 包的主入口，负责：
--   1. 向 cn_vertical 的 hooks 系统注册版心相关回调
--   2. 管理版心配置参数
--   3. 协调 render_banxin.lua 和 render_yuwei.lua 模块
--
-- 【设计原理 / Design Principle】
-- cn_banxin 作为独立包，通过 cn_vertical.hooks 接口与竖排引擎通信：
...
-- Ensure cn_vertical global namespace exists
_G.cn_vertical = _G.cn_vertical or {}
_G.cn_vertical.hooks = _G.cn_vertical.hooks or {}

-- Create banxin namespace
_G.cn_banxin = _G.cn_banxin or {}
...
_G.cn_banxin.config = config

-- Load render modules (relative path from cn_banxin directory)
local banxin_render = nil
local function get_banxin_render()
    if not banxin_render then
        -- Try loading from cn_banxin directory first
        local success, result = pcall(function()
            return dofile(kpse.find_file("cn_banxin/render_banxin.lua") or "cn_banxin/render_banxin.lua")
        end)
        if success then
            banxin_render = result
        else
            -- Fallback to cn_vertical directory for backward compatibility
...
function _G.cn_banxin.configure(new_config)
...
-- Register hooks with cn_vertical
_G.cn_vertical.hooks.is_reserved_column = is_reserved_column
_G.cn_vertical.hooks.render_reserved_column = render_reserved_column
_G.cn_vertical.hooks.get_reserved_config = get_reserved_config

-- Export module
local banxin = {
    config = config,
    configure = _G.cn_banxin.configure,
