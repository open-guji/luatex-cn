...
function textbox.process_inner_box(box_num, params)
    ...
    -- 为了避免循环依赖，我们通过全局 _G.cn_vertical 访问
    local cn_vertical = _G.cn_vertical
    if not cn_vertical or not cn_vertical.prepare_grid then
        utils.debug_log("[textbox] Error: cn_vertical.prepare_grid not found")
        return
    end
    ...
    -- 调用三阶段流水线
    cn_vertical.prepare_grid(box_num, sub_params)
